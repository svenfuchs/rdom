...
class StyleDeclarationTest < Test::Unit::TestCase
  ...
  def setup
    ...
    window.evaluate("var body = window.document.body")
  end
  ...
  [:background, :clear, :display, :position, :width, :zIndex].each do |name|
    test "ruby: has style property #{name}" do
  ...
  test "ruby: style properties are populated from the element's style attribute" do
    assert_equal 'left', body.style.clear
  end

  test "js: style properties are populated from the element's style attribute" do
    assert_equal 'left', window.evaluate("body.style.clear")
  end

  test "ruby: can set properties on style object (using method call syntax)" do
    body.style.clear = 'both'
    assert_equal 'both', body.style.clear
  end

  test "js: can set properties on style object (using method call syntax)" do
    window.evaluate("body.style.clear = 'both'")
    assert_equal 'both', window.evaluate("body.style.clear")
  end

  test "js: can set properties on style object (using hash access syntax)" do
    window.evaluate("body.style['clear'] = 'both'")
    assert_equal 'both', window.evaluate("body.style.clear")
  end
